<template>
    <div>
        <el-card shadow="hover">
            ...
            <el-table :row-class-name="tableRowClassName" :data="comments" style="width: 100%">
                <el-table-column prop="id" width="50"> </el-table-column>
                <el-table-column prop="tableId" label="帖子id" width="80"> </el-table-column>
                <el-table-column prop="userId" label="用户id" width="80"> </el-table-column>
                <el-table-column prop="userName" label="用户名" width="90"> </el-table-column>
                <el-table-column prop="content" show-overflow-tooltip label="内容" width="300"></el-table-column>
                ...
                <el-row :gutter="20">
                    <el-col :xl="12" :lg="12" :md="24" :sm="24" :xs="24">
                        <el-form-item label="id">
                            <el-input disabled v-model="dialogInfoData.comment.id"></el-input>
                        </el-form-item>

                        <el-form-item label="内容">
                            <el-input type="textarea" :rows="4" v-model="dialogInfoData.comment.content"></el-input>
                        </el-form-item>

                         <el-form-item label="状态">
                            <el-select v-model="dialogInfoData.comment.state" placeholder="帖子状态">
                    ...
                    <el-col :xl="12" :lg="12" :md="24" :sm="24" :xs="24">
                        <el-form-item label="图片列表">
                            <el-input type="textarea" :rows="2" v-model="dialogInfoData.comment.images"></el-input>
                            <div>
                                <el-image
                                    v-for="(item, index) in JSON.parse(
                                        dialogInfoData.comment.images == null ? '[]' : dialogInfoData.comment.images
                                    )"
                                    v-bind:key="index"
                                    style="width: 33%"
                                    :src="IMAGE_URL + item"
...
<script>
import { AdminReportCommentData } from '../../api/AdminReportComment';
...
export default {
    ...
    methods: {
        failReport() {
            AdminReportCommentData.handleReport(this.dialogInfoData.comment.id, false).then((res) => {
        ...
        successReport() {
            AdminReportCommentData.handleReport(this.dialogInfoData.comment.id, true).then((res) => {
                this.$message.success(res.message);
                this.getReportComments();
                this.closeDialogInfo();
            });
        },
        getDialogInfoDatas(commentId) {
            AdminCommentData.comment(commentId).then((res) => {
                this.$set(this.dialogInfoData, 'comment', res.data);
            });

            AdminReportCommentData.getReportByComment(commentId).then((res) => {
        ...
        openDialogInfo(index) {
            ...
            this.getDialogInfoDatas(comment.id);
        ...
        getReporComment(index) {
            AdminReportCommentData.comment(this.comments[index].id).then((res) => {
                this.$set(this.comments, index, res.data);
            });
        },
        getReportComments() {
            if (!this.isSearch) {
                AdminReportCommentData.count(this.pageState).then((res) => {
                    this.commentCount = res.data;
                });

                AdminReportCommentData.pageList(this.pageIndex, this.pageSize, this.pageState).then((res) => {
                    this.comments = res.data;
                });
            } else {
                AdminReportCommentData.searchCount(this.keyword, this.pageState).then((res) => {
                    this.commentCount = res.data;
                });

                AdminReportCommentData.searchList(this.keyword, this.pageIndex, this.pageSize, this.pageState).then((res) => {
                    this.comments = res.data;
                });
            }
        }
